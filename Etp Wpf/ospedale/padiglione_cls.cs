...
namespace ospedale
{
    public class padiglione_cls
    {
        ...
        public List<string> cont_coronavirus()
        {
            // Creo la lista di stringhe
            List<string> cont = new List<string>();
            int i = 0;
            // Stringa che uso come output 
            string output;
            // itera per ogni reparto
            foreach(string reparto in reparti_cnt)
            {
                // Concateno uno standard con gli output: Reparto (nome) contagi (n^contagi)
                output = string.Concat("Reparto ", reparto, " contagi ", reparti[i].conta_virus());
                // Aggiungo alla lista l'output
                cont.Add(output);
                i += 1;
            }
            // Ritorno l'output
